...
public class SampleController {
	...
	public Mono<String> sample1() {
		...
		Mono<String> ms1 = webClient.get().uri("sample-service2-v2.charles3.svc.cluster.local").retrieve().bodyToMono(String.class);
// 		Mono<String> ms2 = webClient.get().uri("http://localhost:9003/").retrieve().bodyToMono(String.class);
...
// 		Mono<String> ms8 = webClient.get().uri("http://localhost:9009/").retrieve().bodyToMono(String.class);
		return Mono.zip(ms1).flatMap(data -> {
